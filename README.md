...
<p align="center">
  ...
  <img alt="Version" src="https://img.shields.io/badge/version-0.3-8A63D2">
</p>
...
현재 `CRT 0.3`은 **사용자가 자신의 API Key를 연결해 자기 Mac에서 실행하는 실시간 감지 베타**입니다. 무료 Alpaca IEX에서는 관심종목 감지를 시험할 수 있고, 사용자가 유료 SIP 권한을 가진 경우 전체 상장주 감지 모드를 선택할 수 있습니다. 경보는 가격 움직임 관측이며 투자 추천이나 자동매매 신호가 아닙니다. 내부 빌드 버전은 `0.3.0`입니다.
...
| 실시간 급등 감지 | Alpaca WebSocket IEX/SIP | 초 단위 가격·거래대금 조건을 검사해 감지 로그와 Mac 알림 표시 |
| 데이터 범위 선택 | 사용자 계정 권한 | 무료 IEX 관심종목 또는 유료 SIP 전체 상장주 모드 선택 |
...
2026년 5월 26일 확인 기준, Alpaca Basic에서 구독 없이 사용할 수 있는 주식 실시간 피드는 IEX이며 WebSocket은 최대 30개 종목입니다. 유료 Algo Trader Plus는 SIP 실시간 데이터와 무제한 WebSocket 종목 구독을 제공합니다. `CRT 0.3`은 서버가 시세를 재배포하지 않고 사용자의 키로 로컬에서 직접 수신하도록 설계했습니다. Massive Stocks Basic 사후 스캔은 분당 호출 제한 때문에 상세 분봉 확인을 상위 후보 4개로 제한합니다.
...
- `CRT 0.3`은 실시간 가격 조건 충족 시 즉시 Mac 알림을 생성합니다. 알림은 가격 기반 후보 경보이며 매매 지시가 아닙니다.
