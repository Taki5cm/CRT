...
현재 `CRT 0.2`는 **실시간 매매 신호 앱이 아니라 지난 거래일 분석을 중심으로 한 기능형 베타**입니다. 무료 Alpaca IEX 연결로 관심종목의 현재 체결 가격을 미리 볼 수 있지만, 이는 전체시장 급등 감지나 투자 신호가 아닙니다. 내부 빌드 버전은 `0.2.0`입니다.
...
| 실시간 가격 미리보기 | Alpaca WebSocket IEX | 입력한 관심종목 최대 30개의 IEX 체결 가격을 실시간 표시 |
...
| 관심종목 현재 가격 미리보기 | Alpaca API Key / Secret Key | 무료 IEX 실시간 스트림, 최대 30개 종목 |
| 공식 공시 연결 | 연락 이메일 | SEC EDGAR 요청의 사용자 식별용 헤더에 사용 |

2026년 5월 26일 확인 기준, Massive Stocks Basic은 무료 과거 데이터와 분봉 집계를 제공하지만 분당 호출 제한이 있으므로 상세 분봉 확인을 상위 후보 4개로 제한합니다. Alpaca Basic에서 구독 없이 사용할 수 있는 주식 피드는 IEX이므로 CRT의 관심종목 과거 분봉과 실시간 미리보기는 모두 IEX 기준입니다. IEX WebSocket 구독은 최대 30종목이며, 전체 미국 거래소 실시간 감시에는 유료 전체시장 피드가 필요합니다.
...
- 앱의 `실시간 연결`은 관심종목 가격을 표시하는 IEX 미리보기이며, 아직 급등 자동 알림과 원인 조사 실행을 연결하지 않습니다.
...
- 실시간 전체시장 모니터링, 실시간 급등 알림 또는 자동매매를 제공하지 않습니다.
- 무료 실시간 가격 미리보기는 IEX 체결만 포함하므로 전체시장 현재가와 다를 수 있습니다.
...
| `CRT 0.2` | 직접 날짜 입력·달력 선택, 결과 필터, 분석 완료 알림, IEX 관심종목 실시간 가격 미리보기 |
| 다음 단계 | 급등 자동 감지와 뉴스·공시 후속 조사 연결, 가격 흐름·근거 타임라인 |
