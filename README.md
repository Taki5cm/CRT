...
2026년 5월 26일 확인 기준, Massive Stocks Basic은 무료 과거 데이터와 분봉 집계를 제공하지만 분당 호출 제한이 있으므로 상세 분봉 확인을 상위 후보 4개로 제한합니다. Alpaca Basic의 무료 실시간 데이터는 IEX 거래소에 한정되고 WebSocket 구독은 최대 30종목입니다. 전체 미국 거래소 실시간 감시에는 Alpaca의 유료 전체시장 피드 또는 Massive Stocks Advanced와 같은 유료 데이터가 필요합니다.
