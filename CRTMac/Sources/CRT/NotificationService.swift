...
final class NotificationService: NSObject, UNUserNotificationCenterDelegate {
    ...
    func sendLiveAlertNotification(_ alert: LiveAlert) async {
        ...
        let content = UNMutableNotificationContent()
        content.sound = .default
        content.title = "CRT 0.10 \(alert.direction.label) 포착: \(alert.symbol)"
    ...
    func sendCatalystReportNotification(symbol: String, report: CatalystResearchReport) async {
        guard await permissionState() == .allowed else { return }

        let content = UNMutableNotificationContent()
        content.sound = .default
        content.title = "CRT 0.10 2차 보고: \(symbol)"
