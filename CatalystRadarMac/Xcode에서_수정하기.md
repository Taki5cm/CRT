...
이제부터 앱의 작업 원본은 `CatalystRadar.xcodeproj`입니다. 브라우저 데모가 아니라 이 Xcode 프로젝트를 열어 화면과 기능을 직접 확인하고 수정할 수 있습니다.

## 처음 열기

1. Finder에서 `CatalystRadarMac` 폴더를 엽니다.
2. `CatalystRadar.xcodeproj` 파일을 더블클릭합니다.
3. Xcode 왼쪽 목록에서 `CatalystRadar` 폴더를 펼칩니다.
...
| 앱이 처음 시작되는 방식 | `CatalystRadarApp.swift` | 앱 진입점 |
...
앞으로 기능을 추가하거나 고칠 때도 `CatalystRadar.xcodeproj`에서 바로 보이는 Swift 파일 안에 반영합니다. 즉, 사용자는 Xcode에서 같은 원본 코드를 열어 제 작업을 확인하고, 직접 수정하고, 실행해볼 수 있습니다.

`build` 폴더의 `.app` 파일은 완성된 실행본이라 수정 대상이 아닙니다. 실제로 손대는 곳은 `Sources/CatalystRadar` 안의 `.swift` 파일들입니다.
